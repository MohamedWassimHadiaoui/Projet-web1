...
/**
 * Like buttons handling (AJAX fallback) - updates counts inline
 */
function initLikeButtons() {
    document.addEventListener('click', function(e) {
        ...
        // Build URL, force ajax=1
        const href = btn.getAttribute('href') || btn.dataset.href;
        if (!href) { btn.dataset.disabled = '0'; return; }
        ...
        fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            ...
            .then(payload => {
                if (payload && payload.success) {
                    ...
                    if (countEl) countEl.textContent = payload.likes;
                    // small visual feedback
                    btn.classList.add('liked');
                    setTimeout(() => btn.classList.remove('liked'), 700);
                } else {
                    // basic error ui
            ...
            .finally(() => { btn.dataset.disabled = '0'; });
    });
}
 ...
 */
function initTabs() {
    // support existing structure (.tabs-container .tab-item)
    ...
    // support .tabs-nav + .tab-btn pattern used in events_and_contenus
    ...
    tabNavs.forEach(nav => {
        ...
        buttons.forEach(btn => {
            btn.addEventListener('click', function(e) {
                ...
                // hide all .tab-content under the same container
                const container = nav.closest('.container');
                if (!container) return;
                const contents = container.querySelectorAll('.tab-content');
                contents.forEach(c => c.classList.remove('active'));
