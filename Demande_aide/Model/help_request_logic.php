...
function hr_create(array $data)
{
    ...
    $stmt = $pdo->prepare('INSERT INTO help_requests (help_type, urgency_level, situation, location, contact_method, status) VALUES (?, ?, ?, ?, ?, ?)');
    $stmt->execute([
        $payload['help_type'],
...
function hr_update($id, array $data)
{
    ...
    $stmt = $pdo->prepare('UPDATE help_requests SET help_type = ?, urgency_level = ?, situation = ?, location = ?, contact_method = ?, status = ? WHERE id = ?');
    return $stmt->execute([
        $payload['help_type'],
        $payload['urgency_level'],
        $payload['situation'],
...
{
    $fields = ['help_type', 'urgency_level', 'situation', 'location', 'contact_method', 'status'];
    $data = [];
...
/**
 * Récupère les demandes avec pagination
 */
function hr_get_paginated($limit, $offset)
{
    $pdo = getDb();
    $stmt = $pdo->prepare('SELECT * FROM help_requests ORDER BY created_at DESC LIMIT :limit OFFSET :offset');
...
/**
 * Compte le nombre total de demandes par statut
 */
function hr_count_by_status($status)
{
    $pdo = getDb();
    $stmt = $pdo->prepare('SELECT COUNT(*) FROM help_requests WHERE status = ?');
    $stmt->execute([$status]);
    return $stmt->fetchColumn();
}
