...
/**
 * Récupère les organisations avec pagination
 */
function org_get_paginated($limit, $offset)
{
    $pdo = getDb();
    $stmt = $pdo->prepare('SELECT * FROM organisations ORDER BY created_at DESC LIMIT :limit OFFSET :offset');
...
/**
 * Compte le nombre total d'organisations
 */
function org_count_all()
{
    $pdo = getDb();
...
/**
 * Compte le nombre total d'organisations par statut
 */
function org_count_by_status($status)
{
    $pdo = getDb();
    $stmt = $pdo->prepare('SELECT COUNT(*) FROM organisations WHERE status = ?');
    $stmt->execute([$status]);
    return $stmt->fetchColumn();
}
