...
class PublicationController {
    ...
    public function listPublications() {
        try {
            $sql = "SELECT * FROM publication WHERE statut = 'approved' ORDER BY date_creation DESC";
    ...
    public function listPendingPublications() {
        try {
            $sql = "SELECT * FROM publication WHERE statut = 'pending' ORDER BY date_creation DESC";
            $stmt = $this->pdo->query($sql);
            $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
            ...
            error_log("Erreur lors de la récupération des publications en attente : " . $e->getMessage());
            return false;
        }
    }
    ...
    public function listAllPublications() {
        try {
            ...
            error_log("Erreur lors de la récupération de toutes les publications : " . $e->getMessage());
            return false;
        }
    }
    
    public function approvePublication($id_publication) {
        try {
            $sql = "UPDATE publication SET statut = 'approved' WHERE id_publication = :id";
            $stmt = $this->pdo->prepare($sql);
            $stmt->bindValue(':id', $id_publication, PDO::PARAM_INT);
            return $stmt->execute();
        } catch (PDOException $e) {
            error_log("Erreur lors de l'approbation de la publication : " . $e->getMessage());
            return false;
        }
    }
    
    public function rejectPublication($id_publication) {
        try {
            $sql = "UPDATE publication SET statut = 'rejected' WHERE id_publication = :id";
            $stmt = $this->pdo->prepare($sql);
            $stmt->bindValue(':id', $id_publication, PDO::PARAM_INT);
            return $stmt->execute();
        } catch (PDOException $e) {
            error_log("Erreur lors du rejet de la publication : " . $e->getMessage());
            return false;
        }
    }
    
    public function countPendingPublications() {
        try {
            $sql = "SELECT COUNT(*) as count FROM publication WHERE statut = 'pending'";
            $stmt = $this->pdo->query($sql);
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            return $result['count'];
        } catch (PDOException $e) {
            error_log("Erreur lors du comptage des publications en attente : " . $e->getMessage());
            return 0;
        }
    }
