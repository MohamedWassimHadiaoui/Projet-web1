...
/**
 * Vérifie si une chaîne contient des caractères spéciaux non autorisés
 * Autorise : lettres (majuscules et minuscules), chiffres, espaces, accents, tirets, apostrophes
 * @param {string} text - Le texte à valider
 * @returns {boolean} - true si contient des caractères spéciaux non autorisés, false sinon
 */
function containsSpecialCharacters(text) {
    ...
    // Expression régulière : autorise lettres (avec accents), chiffres, espaces, tirets, apostrophes
    // Caractères autorisés :
    // - Lettres : a-z, A-Z
    // - Chiffres : 0-9
    // - Espaces : \s
    // - Accents français : àáâãäåæçèéêëìíîïðñòóôõöùúûüýÿ
    // - Tirets : -
    // - Apostrophes : '
...
/**
 * Valide qu'un texte ne contient pas de caractères spéciaux
 * @param {string} text - Le texte à valider
...
/**
 * Vérifie si un texte contient uniquement des lettres (avec accents, espaces, tirets, apostrophes)
 * Utilisé pour valider les noms - PAS de chiffres ni caractères spéciaux
 * @param {string} text - Le texte à valider
 * @returns {boolean} - true si contient des caractères non autorisés (chiffres ou caractères spéciaux), false sinon
 */
function containsNonLetters(text) {
    ...
    // Expression régulière : autorise UNIQUEMENT lettres (avec accents), espaces, tirets, apostrophes
    // Caractères autorisés :
    // - Lettres : a-z, A-Z
    // - Espaces : \s
    // - Accents français : àáâãäåæçèéêëìíîïðñòóôõöùúûüýÿ
    // - Tirets : -
    // - Apostrophes : '
    // PAS de chiffres (0-9) ni autres caractères spéciaux
    const regex = /^[a-zA-Z\sàáâãäåæçèéêëìíîïðñòóôõöùúûüýÿ\-']+$/;
    return !regex.test(text);
}

/**
 * Valide qu'un texte contient uniquement des lettres (pour les noms)
 * @param {string} text - Le texte à valider
 * @returns {object} - {valid: boolean, message: string}
 */
